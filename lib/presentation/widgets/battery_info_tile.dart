...
class _BatteryInfoTileState  extends State<BatteryInfoTile>  {
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          child: isLinked ? _linkedUI(context,bsn, mac, charge*1.0, voltage, temperature) : _notLinkedUI(context),
        ),
      ),
    );
  }
  ...
  // ================= LINKED UI =================
  Widget _linkedUI(BuildContext context,String bsn, String mac, double charge, int voltage, int temperature) {
    return Column(
...
await widget.macController.deleteTrackr(widget.data["index"]);
                setState(() {
                  isLinking=false;
          ...
          child: isLinking
            ...
            onPressed: () async {
              final mac = await QRScannerWidget(
                context,
                "QR Code Battery ${widget.data["index"]}",
              );
